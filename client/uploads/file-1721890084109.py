...
Aryan Kumar, ak2488 & Elena Caplinger, ekc75
October 27th 2023
"""
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_iteration(n), report_error('Invalid iterations',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_iteration(n), report_error('not a valid iteration',n)
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert type(k) == int and k >= 1, report_error('k is invalid',k)
    assert type(n) == int and n>=3, report_error('n is invalid',n)


    # HINT: w.clear() clears window.
    # HINT: Set the visible attribute to False at the end, and remember to flush
    w.clear()
    t = Turtle(w,(0,0),'red',180)
    t.speed = sp
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    assert k >=1, report_error('k is less than 1',k)
    assert type(n) == int, report_error('n is not an int',n)
    assert n >= 3, report_error('n is less than 3',n)
...
def draw_polygon(t, side, n):
    ...
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert type(n) == int and n >= 1, report_error('n is invalid', n)
...
def radiate_petals_helper(t, radius, width, n, sp):
    ...
    #    even though all the examples use strings with color n
    pass
# HELPER FUNCTION. DO NOT MODIFY
def draw_diamond(t, length, width):
    ...
    assert is_valid_length(width),  report_error('width is invalid',width)
...
def tsquare(w, side, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('s is not a valid length')
    assert is_valid_speed(sp), report_error('sp is not a valid speed')
...
def tsquare_helper(p, x, y, side, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('x is not a valid coordniate',x)
    assert is_number(y), report_error('y is not a valid coordniate',y)
    assert is_valid_length(side), report_error('side is invalid',side)
    assert is_valid_depth(d), report_error('depth is invalid',d)
    ...
    if d == 0:
        ...
        tsquare_helper(p,x,y,side,d-1)
        tsquare_helper(p,x-side/2.0,y+side/2.0,side/2.0,d-1)
        tsquare_helper(p,x+side/2.0,y+side/2.0,side/2.0,d-1)
        tsquare_helper(p,x-side/2.0,y-side/2.0,side/2.0,d-1)
        tsquare_helper(p,x+side/2.0,y-side/2.0,side/2.0,d-1)
...
def hexaflake(w, side, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('Invalid side length')
    assert is_valid_depth(d), report_error('Invalid depth')
    assert is_valid_speed(sp), report_error('Invalid speed')
...
def hexaflake_helper(p, x, y, side, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('x coordinate is invalid',x)
    assert is_number(y), report_error('y coordinate is invalid',y)
    assert is_valid_length(side), report_error('invalid length',side)
    ...
    if d == 0:
        ...
        #top right
        hexaflake_helper(p, x+side/3, y+(math.sqrt(3)*side/3), side/3, d-1)
        #top left
        hexaflake_helper(p, x-side/3, y+(math.sqrt(3)*side/3), side/3, d-1)
        #right
        hexaflake_helper(p, x+side*2/3, y, side/3, d-1)
        #left
        hexaflake_helper(p, (x-side*2/3), y, side/3, d-1)
        #bottom right
        hexaflake_helper(p, x+side/3, y-math.sqrt(3)*side/3, side/3, d-1)
...
def kochflake(w, side, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('side not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    ...
    t.color = 'blue'
    t.speed = sp
    t.move(-side/2,-side/(2*math.sqrt(3)))
    t.heading = 60
    for i in range(3):
